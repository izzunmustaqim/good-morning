package com.applab.goodmorning.Login.provider;
...
{
    private static String CONTENT_AUTHORITY = "com.applab.goodmorning.login.provider";
    ...
    public void setUriMatcher() {
        ...
        uriMatcher.addURI(CONTENT_AUTHORITY, PATH_CONTACT, TOKEN);
        uriMatcher.addURI(CONTENT_AUTHORITY, PATH_CONTACT + "/#", TOKEN_ID);
    }
    ...
    public boolean onCreate() {
        CONTENT_AUTHORITY = getContext().getResources().getString(R.string.token_provider);
            ...
            case TOKEN:
                ...
                try {
                    for (ContentValues value : values) {
                        long _id = db.insert(dbHelper.TOKEN_TABLE_NAME, null, value);
                        if (_id != -1) {
                            numRowsInsert++;
                        }
                    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case TOKEN:
                rowID = db.insert(dbHelper.TOKEN_TABLE_NAME, "", values);
                if (rowID > 0) {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case TOKEN:
                rowDeleted = db.delete(dbHelper.TOKEN_TABLE_NAME, selection, selectionArgs);
                break;
            case TOKEN_ID:
                String id = uri.getPathSegments().get(1);
                rowDeleted = db.delete(dbHelper.TOKEN_TABLE_NAME, dbHelper.TOKEN_COLUMN_ID + " = " + id +
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case TOKEN:
                count = db.update(dbHelper.TOKEN_TABLE_NAME, values,
                        selection, selectionArgs);
                break;
            case TOKEN_ID:
                count = db.update(dbHelper.TOKEN_TABLE_NAME, values, dbHelper.TOKEN_COLUMN_ID +
