package com.applab.goodmorning.Products.provider;
...
public class ProductProvider extends ContentProvider {
    private static String CONTENT_AUTHORITY = "com.applab.goodmorning.product.provider";
    ...
    public void setUriMatcher() {
        ...
        uriMatcher.addURI(CONTENT_AUTHORITY, PATH_CONTACT, PRODUCT);
        uriMatcher.addURI(CONTENT_AUTHORITY, PATH_CONTACT + "/#", PRODUCT_ID);
    }
    ...
    public boolean onCreate() {
        CONTENT_AUTHORITY = getContext().getResources().getString(R.string.product_provider);
            ...
            case PRODUCT:
                ...
                try {
                    for (ContentValues value : values) {
                        long _id = db.insert(dbHelper.PRODUCT_TABLE_NAME, null, value);
                        if (_id != -1) {
                            numRowsInsert++;
                        }
                    }
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case PRODUCT:
                rowID = db.insert(dbHelper.PRODUCT_TABLE_NAME, "", values);
                if (rowID > 0) {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case PRODUCT:
                rowDeleted = db.delete(dbHelper.PRODUCT_TABLE_NAME, selection, selectionArgs);
                break;
            case PRODUCT_ID:
                String id = uri.getPathSegments().get(1);
                rowDeleted = db.delete(dbHelper.PRODUCT_TABLE_NAME, dbHelper.PRODUCT_COLUMN_ID + " = " + id +
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case PRODUCT:
                count = db.update(dbHelper.PRODUCT_TABLE_NAME, values,
                        selection, selectionArgs);
                break;
            case PRODUCT_ID:
                count = db.update(dbHelper.PRODUCT_TABLE_NAME, values, dbHelper.PRODUCT_COLUMN_ID +
